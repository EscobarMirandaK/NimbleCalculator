...
namespace Tests
{
    public class CalculatorTests
    {
        ...
        [Fact]
        public void SubtractionOperation_ReturnsDifference()
        {
            var calculator = new Calculator();
            var input = "5,3,2";
            var operation = OperationType.Substraction;
            (int result, string formula) = calculator.Calculate(input, operation);
            Assert.Equal(0, result);
            Assert.Equal("5 - 3 - 2 = 0", formula);
        }

        [Fact]
        public void MultiplicationOperation_ReturnsDifference()
        {
            var calculator = new Calculator();
            var input = "2,3,4";
            var operation = OperationType.Multiplication;
            (int result, string formula) = calculator.Calculate(input, operation);
            Assert.Equal(24, result);
            Assert.Equal("2 * 3 * 4 = 24", formula);
        }

        [Fact]
        public void DivisionOperation_ReturnsQuotient()
        {
            var calculator = new Calculator();
            var input = "100,2,5";
            var operation = OperationType.Division;
            (int result, string formula) = calculator.Calculate(input, operation);
            Assert.Equal(10, result);
            Assert.Equal("100 / 2 / 5 = 10", formula);
        }
        ...
        public void DivisionByZero_ThrowsException()
        {
            ...
            var input = "100,0,5";
            var operation = OperationType.Division;
            var exception = Assert.Throws<DivideByZeroException>(() => calculator.Calculate(input, operation));
